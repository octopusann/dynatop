...
// [[Rcpp::export]]
void dt_bexp_pet_courant(Rcpp::DataFrame hillslope, // hillslope data frame
...
// [[Rcpp::export]]
void dt_bexp_pet_init(Rcpp::DataFrame hillslope, // hillslope data frame
...
// [[Rcpp::export]]
void dt_bexp_pet_implicit(Rcpp::DataFrame hillslope, // hillslope data frame
			  Rcpp::DataFrame channel, // channel data frame
			  Rcpp::DataFrame flow_direction, // flow directions data frame
  ...
  for(int it = 0; it < obs.nrow(); ++it) {
	...
	// compute the revised evapotranspiration
	double a_p = std::min( pet[cid], ( s_rz[ii] + Dt*(precip[cid] + r_sf_rz - r_rz_uz)) );
	
    	// compute revised r_sf_rz
    	r_sf_rz = std::min( r_sf_rz,
    			    (s_rz_max[ii] - s_rz[ii] - Dt*(precip[cid]-a_p-r_rz_uz))/Dt
    			    );

	// solve for root zone
	s_rz[ii] = s_rz[ii] + Dt*(precip[cid] + r_sf_rz - r_rz_uz - a_p) ;

    	// solve for surface
    	s_sf[ii] = ( Dx[ii] / (Dx[ii] + Dt*c_sf[ii]) ) * ( s_sf[ii] + Dt*( (q_sf_in[cid]/area[ii]) - r_sf_rz ));
	
    	// apply pet loss and precip input to mass balance
    	mbv[1] -= a_p*area[ii]*Dt;
